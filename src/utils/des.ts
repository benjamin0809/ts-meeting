...
export class DesHelper {
  ...
  /// iv          初始化向量
  static des (beinetkey: string, message: string, encrypt: number, mode: number, iv: string) {
    ...
    const keys = this.des_createKeys(beinetkey)
    let m = 0
    let i = 0
    let j = 0
    let temp = 0
    ...
    const iterations = keys.length === 32 ? 3 : 9 // s ingle or triple des
  ...
  static des_createKeys (beinetkey: string) {
    // declaring this locally speeds things up a bit
    ...
    const iterations = beinetkey.length >= 24 ? 3 : 1
    ...
    for (let j = 0; j < iterations; j++) {// either 1 or 3 iterations
      let left = (beinetkey.charCodeAt(m++) << 24) | (beinetkey.charCodeAt(m++) << 16) | (beinetkey.charCodeAt(m++) << 8) | beinetkey.charCodeAt(m++)
      let right = (beinetkey.charCodeAt(m++) << 24) | (beinetkey.charCodeAt(m++) << 16) | (beinetkey.charCodeAt(m++) << 8) | beinetkey.charCodeAt(m++)
  ...
  static HexTostring (s: string): string {
    ...
    for (let i = 0; i < s.length; i += 2) {
      const sxx = parseInt(s.substring(i, i + 2), 16)
